      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Fallback indicator — the metrics are real, the wording is templated. */}
            {suggestion.aiGenerated === false && (
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <span
                      className="inline-flex items-center gap-1 rounded-full bg-muted/40 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground/80 ring-1 ring-inset ring-white/[0.06] cursor-help"
                      aria-label={t('playbook.templateBadge', 'Template')}
                    >
                      <FileText className="h-2.5 w-2.5" />
                      {t('playbook.templateBadge', 'Template')}
                    </span>
                  </TooltipTrigger>
                  <TooltipContent side="top" className="max-w-xs">
                    <p>
                      {t(
                        'playbook.templateTooltip',
                        'AI provider was unavailable — wording comes from a template. The underlying metrics are real.',
                      )}
                    </p>
                  </TooltipContent>
                </Tooltip>
